...
def run_simulation(n_days):
    # Define parameters
    VS, VM, VL = 20, 15, 10
    XCake, XChoc = 0.05, 0.10

    # Simulate values
    xVS = np.random.poisson(VS, n_days)
    xVM = np.random.poisson(VM, n_days)
    xVL = np.random.poisson(VL, n_days)
    xVCake = np.random.binomial(xVS + xVM + xVL, XCake, n_days)
    xVChoc = np.random.binomial(xVS + xVM + xVL, XChoc, n_days)
    ...
    # Apply the function and get results
...
def sensitivity_analysis(FUN, X):
    ...
    for i in range(n):
        ...
        new_xvals[i] *= 1.01  # Perturb the i-th variable by 1%
        new_y = FUN(*new_xvals)
